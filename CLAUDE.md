...
| `spacetime_constructor.html` | Interactive 3-D tool ([live](https://jimscarver.github.io/quantum-logical-framework/spacetime_constructor.html)): space = node position, time = clock rate, both = frequency shown as colour; drop masses (redshift = time dilation); spawn e/μ/p/n from the vacuum as balanced pairs (a lone charge can't close ZFA). **No forces** — bodies drift down the `w_ZFA` latency gradient (the possibilist gradient, mass-independent `−∇w`), so bound states are *generated by the dynamics*: free p + e recombine into H (emitting a photon), two H bond into H₂, H + H̄ annihilate; decays/annihilation throw off photon streaks. A **Temperature** slider (absolute zero → CMB default → Planck ~10³² K) drives thermal photons + threshold pair production; near the Planck top **black holes form** (Compton = Schwarzschild) and Hawking-evaporate into hadrons (a hadron IS a quantum BH). **Click identifies** by frequency + spin; **matter/antimatter balance** stays 0 (all processes conserve; baryogenesis absent). Decay is deterministic census-exponential (`Decay.md` §1a). **No probability — the closure census draws the space**: each node placed by a 3-axis ZFA closure's own walk (`QLF_CensusBrownian`; Genesis's three spatial pairs), "continuum one closure at a time" — no forces (latency gradient), no dice (`QLF_BornProbability`). Dependency-free canvas, mobile. Full write-up `SpaceTime.md` §7 |
