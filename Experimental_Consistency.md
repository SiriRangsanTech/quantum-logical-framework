...
| **Protein folding** | A conformation **is** a twist history (the six signed axis steps *are* the six spatial twists), and a **contact is a ZFA closure**: segment plus contact edge has zero net displacement, hence count-balanced, hence Pauli-closed by the keystone — both conjuncts, the second free. The lattice-protein **parity rule** (contacts only at odd sequence separation) follows from count balance rather than being a lattice artefact, and the contact energy is the substrate's own `log 2` rather than a fitted `ε`. Checks: self-avoiding-walk counts reproduce OEIS A001411/A001412; loop-closure multiplicity falls `0.122 → 0.013` from span 3 to 9 (**contact order, counted**); the 4×4 designability census reproduces Li–Helling–Tang–Wingreen (69 structures, designability 21–521, 21.8% of sequences folding). **Proven no-go:** the mirror is a closure-preserving bijection, so counting cannot select a handedness — homochirality is upstream | `QLF_Folding.lean` (`contact_is_closure`, `contact_separation_odd`, `map_mirror_bijective`); [`Protein_Folding.md`](Protein_Folding.md), [`protein_census.py`](protein_census.py) (10 invariants, in CI) |
