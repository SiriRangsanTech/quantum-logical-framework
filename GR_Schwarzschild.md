...
- **Gravitational waves — partially anchored.** The static metric here does not give the *wave*. But several features of a GW follow from existing substrate machinery and are Lean-anchored in [`lean/QLF_GravitationalWaves.lean`](lean/QLF_GravitationalWaves.lean): a GW carries no gauge fold, so it is **massless** and propagates at **`c = L_Planck/τ_Planck`** (`gw_speed_eq_planck_ratio` — the GW170817 result `|v_GW − c|/c < 10⁻¹⁵`); the **graviton is spin-2**, a composite of four half-spins = two photon-worths (`graviton_integer_spin`, reusing `boson_even_pairs` from [`QLF_Spin`](lean/QLF_Spin.lean)); and it has **2 transverse polarizations** (the `±2` helicities, not `2J+1=5`) as a masslessness consequence, exactly like the photon's 2 (`massless_two_polarizations`). **The linearized wave equation is now anchored via the density-perturbation route:** a GW is a propagating modulation `δρ` of the closure-density field around the SOC equilibrium `ρ*` (`QLF_SteadyStateDensity`), and the discrete d'Alembertian `□_d = ∂_t² − ∂_x²` annihilates every traveling-wave profile — `boxD_rightMover`/`boxD_leftMover`/`boxD_dAlembert` — so `□_d δρ = 0` at one Planck length per Planck tick `= c`, with the metric perturbation `h ∝ δρ/ρ*` obeying the same equation (`boxD_metricPerturbation`); the continuum limit is `□h_μν = 0`. The **quadrupole is the leading radiative multipole** (`quadrupole_is_leading_radiative`): mass–energy and momentum conservation force the monopole and dipole to be non-radiative (`ddt2 = 0`), the quadrupole not (`= 2`). Numerical companion: [`gw_density_wave.py`](gw_density_wave.py) (traveling-wave residual `~10⁻¹⁵`, pulse speed `1.0000` cell/tick, the multipole selection). What is **still open** (`gravitational_waves_in_progress`), localized to the dynamical-metric step: deriving the operator `boxD` from the SOC binding+continuity rate equations (`QLF_ClosureAttraction`/`QLF_SteadyStateDensity`) in the continuum limit — the same gap as the full Einstein field equations — and the quadrupole luminosity *coefficient* `G/(5c⁵)` in `L = (G/5c⁵)⟨d³Q/dt³⟩²` (with `G = L_P²c³/ℏ` and `8π = 4π·2` already substrate-fixed).
