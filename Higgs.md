...
So `m_t` is derived **structurally** — the electroweak *order parameter*, `= v/√2`, its `O(1)` coefficient fixed-point-forced, the only quark whose condensate is unconfined (`QLF_QuarkMass`). What stays open is its **absolute** value: the top-condensation scale `v = R_stable` is *self-generated* (like `Λ_QCD` in QCD — the constituent scale sets itself where the effective coupling goes critical), and it does not yet land on a clean transmutation exponent (`ln(M_Pl/m_t) ≈ 38.8`, not a QLF `nπ`). Deriving `m_t` absolutely *is* deriving that condensation scale — the one open electroweak number (`higgs_turbulence_in_progress`).
