...
So the reduction bottoms out honestly: **the whole Standard-Model mass spectrum collapses to two scales** — the Planck floor (by construction, `μ² = 1/2`, [`QLF_PlanckScale`](lean/QLF_PlanckScale.lean)) and *one* dynamically-generated low scale — of which QLF derives the QCD/proton scale (`14π`) and leaves the electroweak `v` as the **relevant-operator anchor** (= `R_stable`, condensation-open). Deriving `v` *is* deriving the turbulent condensation depth; it is the one genuine open number of the electroweak sector, and it is a relevant-operator scale rather than a transmutation one — which is *why* it is the anchor, not a derived ratio.
