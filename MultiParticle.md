...
## See also — the interactive picture

[`spacetime_constructor.html`](spacetime_constructor.html) ([live](https://rchain-community.github.io/quantum-logical-framework/spacetime_constructor.html)) is the browser companion to this engine: the same shared-closure / latency-field ideas, drawn instead of computed. It renders space as node position, time as clock rate, and both as frequency-coloured light, lets you drop masses (redshift = time dilation) and spawn particles from the vacuum as balanced pairs that decay and annihilate. See [`SpaceTime.md` §7](SpaceTime.md).
