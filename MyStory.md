...
- **Information is physically ½-spin — Wheeler's "it from bit," now a theorem.** The half-spin I puzzled over as a student — each √−1 folding as half-spin, the photon as two halves adding to one — turns out to be the atom of information itself: a *two-valued* spin-½ closure carries exactly one bit (`log 2`) while a *single-valued* object carries **none**, the 360°→720° double-valuedness proven from the explicit rotation matrices, grounding Élie Cartan's 1913 spinor as the carrier of a bit. The torch Wheeler handed me at his 90th birthday — *it from bit* — is machine-verified ([Mathematics_From_QLF.md](Mathematics_From_QLF.md) §Rung 5a, `lean/QLF_SpinorInformation.lean`).
