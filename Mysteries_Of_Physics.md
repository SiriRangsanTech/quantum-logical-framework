...
| **The Planck epoch / primordial black holes** | At the highest energies a closure crosses **Compton = Schwarzschild** ⟹ **Planck-mass black holes** that Hawking-cascade into **hadrons** (a hadron *is* a quantum black hole) — QLF's *logical bang* (`Creation.md` §8a). The [interactive constructor](https://jimscarver.github.io/quantum-logical-framework/spacetime_constructor.html) reproduces the whole cascade — primordial BHs → hadrons → atoms — from ZFA + the closure census alone, nothing scripted ([▶ heat to the Planck top and watch it cascade](https://jimscarver.github.io/quantum-logical-framework/spacetime_constructor.html#t=0.95)); the `μ²=1/2` crossing is computed in [`genesis.py`](genesis.py) §7 | ✅ structural / ⚪ PBH abundance | `QLF_QuantumBlackHole.lean`, `QLF_PlanckScale.lean`, [`Creation.md`](Creation.md) §8a, [`SpaceTime.md`](SpaceTime.md) §7, [`genesis.py`](genesis.py) §7 |
...
| **Primordial nucleosynthesis (`Y_p`)** | Every surviving neutron → ⁴He ⟹ `Y_p = 2r/(1+r)`; freeze-out `r=1/7 ⟹ Y_p=1/4` (matches 0.247). [▶ see an α (⁴He) closure](https://jimscarver.github.io/quantum-logical-framework/spacetime_constructor.html#qc=alpha%20%40%200%2C0%2C0) | 🔵 (`r`, D/⁷Li open) | `QLF_Nucleosynthesis.lean` |
