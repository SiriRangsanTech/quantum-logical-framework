...
**Two axioms in [`lean/QLF_Riemann.lean`](lean/QLF_Riemann.lean) are bridges, not gaps.** `spectral_hilbert_polya` and `resonant_computation_for` are stated as axioms in Lean because they cross from RCA₀ (where QLF's substrate combinatorics live) into WKL₀/ACA₀ (where ζ's analytic structure lives). Demanding a ZFC-internal proof of `spectral_hilbert_polya` is asking for what BB/Gödel establishes ZFC cannot always provide. What QLF supplies is **structural motivation** for each bridge:
...
- `resonant_computation_for` is the encoding bridge from ℂ-labels to substrate-realisable computations; its content is the substrate-to-analytic mapping that the substrate spectral structure exists to model.
- `NonTrivialZero` used to be listed here as a third bridge. That was a misclassification, and correcting it is a small strengthening rather than a loss: naming ζ's zeros is vocabulary, and it is now the **definition** `riemannZeta ρ = 0 ∧ 0 < ρ.re < 1` over Mathlib's ζ. While the name was opaque, `spectral_hilbert_polya` was satisfied by the interpretation under which no complex number is a non-trivial zero — the statement below held in a model where it said nothing about ζ. It now carries the real content of RH.

The substrate-structural argument provides the **substrate-side conservation law** the two bridge axioms transport into the analytic side. The conservation law itself — *primes balance to 1/2 or they don't cancel at all* — is RCA₀-statable and rigorous within the QLF combinatorial core.
