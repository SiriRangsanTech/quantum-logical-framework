...
**No probability — just logic.** The constructor uses **no randomness**: every "choice" (spawn positions, emission directions, orbit planes) is the next term of a deterministic low-discrepancy golden-ratio sequence — the same `φ` behind QLF's optimal packing. Given the same actions it unfolds identically, consistent with QLF's stance that probability is not a primitive (Born statistics are integer count ratios, [`QLF_BornProbability`](lean/QLF_BornProbability.lean); even decay is deterministic ensemble logic, [`Decay.md`](Decay.md) §1a), and with the substrate being pure logic.
