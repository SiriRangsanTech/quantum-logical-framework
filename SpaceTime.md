...
**Select a frequency band.** Two **log sliders** over rest frequency `f = m/mₑ` (since `E = m = 1/R`) select a band `[f_lo, f_hi]`. The visible spectrum is then *stretched across the band* — band-low → red, band-high → violet — so a narrow band spreads the full rainbow across just those frequencies; closures **outside** the band dim back, and **Capture** takes only the in-band closures on screen. It is a frequency microscope on the census: dial in the leptons, or the hadrons, or the black holes.

**The vacuum foam moves the matter — a ZFA-balanced Brownian walk.** An immersed closure is buffeted by the surrounding vacuum foam, so real particles execute a **random walk on top of their gradient drift** — Einstein's Brownian motion, with the foam as the medium. But the walk is **not dice**: each kick is the step of a **closed census walk** (a ZFA-balanced ±1 history, [`QLF_CensusBrownian`](lean/QLF_CensusBrownian.lean) / [`QLF_Turbulence`](lean/QLF_Turbulence.lean)), drawn from the same census that draws the space. Because the walk is *closed*, its increments **sum to zero over each period** — the net impulse is zero, so the fluctuation adds no net energy: a ZFA-obedient jitter, not a hidden force. The drift is the deterministic mean; the census walk is the vacuum fluctuation about it. (`Vacuum jitter` toggle, default on.)

**One relative perspective — the movable observer.** The whole model is drawn from *one* observer's frame: a stick figure — a Markov blanket at the origin of its own horizon ([`QLF_HorizonClosure`](lean/QLF_HorizonClosure.lean)), with everything seen relative to him. He is the **frame origin**, so every view operation is *his*: **orbit** (drag off him) spins the scene around him, **zoom** homes in *from the observer* (his screen position is the fixed point of the zoom, not the display centre), and **grabbing the observer** (or `Shift`-drag anywhere) **trucks the whole perspective with him** — the observer and the entire scene translate together, because moving the frame origin moves everything relative to it. There is no god's-eye spin: the view holds a fixed orientation unless you turn it (auto-spin is opt-in). This is also why the thermal glow arrives **from the sky inward**: the CMB is the Gibbons–Hawking radiation of the de Sitter **horizon** ([`QLF_HorizonTemperature`](lean/QLF_HorizonTemperature.lean)), emitted on the outer shell and travelling in, not from a source inside.
