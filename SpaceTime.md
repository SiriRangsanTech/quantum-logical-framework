...
**No probability — the variation IS the closure census.** The constructor uses **no randomness**: every "choice" (spawn positions, emission directions, orbit planes) is drawn from the substrate's own combinatorics — the **C(14,7) = 3432 ZFA-balanced closures** of length 14 (a balanced string is a closed walk, [`QLF_CensusBrownian`](lean/QLF_CensusBrownian.lean); the census counts them), read in low-discrepancy order into `[0,1)` and cycled. So the field's structure is generated *by* the closure census, not by a random-number generator or an external constant. This is QLF's stance made literal — probability is not a primitive (Born statistics are integer count ratios, [`QLF_BornProbability`](lean/QLF_BornProbability.lean); even decay is deterministic ensemble logic, [`Decay.md`](Decay.md) §1a) — and the same census that yields `π` (`QLF_PhysicalPi`) here seeds the geometry.
