...
- [**QLF Flow Chart** (live, clickable diagrams + printable PDF)](https://rchain-community.github.io/quantum-logical-framework/FlowChart.html) — the whole framework as a one-page visual map: *one substrate → five families → eleven domains*. Universal Relativity's results are the spacetime/forces/gravity/cosmology domains (1, 3, 5, 6); the *it-from-bit* base of the hypothesis (§3b) sits in the foundational-logic intro and domain 7. Text index: [`FlowChart.md`](FlowChart.md).
